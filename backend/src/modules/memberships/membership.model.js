...
export const getMembershipHistory=async(id)=>{
    const memberships= await pool.query(`select m.id_membership, mt.name, m.init_date , m.end_date,
                                        case
                                            WHEN m.is_active = false AND m.end_date >= CURRENT_DATE THEN 'anulado'
                                            WHEN m.is_active = false AND m.end_date < CURRENT_DATE THEN 'expirado'
                                            else 'activo'
                                        end as state
